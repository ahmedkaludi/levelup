...
class HeaderFooter_Customize_Layout_Builder_Frontend {
    ...
    function render_row( $items, $id = '', $device = 'desktop' ) {
        ...
        while ( $index < $n ) {
            ...
            $merge      = headerfooter_get_setting( $merge_key, $device );
            $merge_next = false;
            $merge_prev = false;
            if ( $merge == 'no' || $merge == '0' ) {
                $merge = false;
            ...
            if ( $next_item ) {
                ...
                $merge_next     = headerfooter_get_setting( $merge_key_next, $device );
            }
            ...
            if ( $show && isset( $this->rows[ $row_id ] ) ) {
                ...
                if ( ! empty( $show_on_devices ) ) {
                    ...
                    if ( ! empty( $desktop_items ) || ! empty( $mobile_items ) ) {
                        ...
                        $row_text_mode = headerfooter_get_setting( $this->id . '_' . $row_id . '_text_mode' );
                        if ( $row_layout ) {
