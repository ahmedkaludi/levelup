...
class OneClickDemoImport {
	...
	/**
	 * The instance of the LEVELUPIMPORT\Importer class.
	 *
	 * @var object
	 */
	public $importer;
  ...
  private $plugin_page_setup = array();
	...
	protected function __construct() {
		...
		add_action( 'wp_ajax_levelup_after_import_data', array( $this, 'after_all_import_data_ajax_callback' ) );
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( 'toplevel_page_levelup' === $hook && $_GET['type']=='template' ) {
			...
			wp_enqueue_script( 'levelup-main-js', LEVELUP_OCDI_URL . 'assets/js/main.js' , array( 'jquery', 'jquery-ui-dialog' ), LEVELUP_VERSION );
			...
			wp_localize_script( 'levelup-main-js', 'levelup_import',
				array(
					...
					'texts'            => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', LEVELUP_TEXT_DOMAIN ),
						'dialog_title'          => esc_html__( 'Are you sure?', LEVELUP_TEXT_DOMAIN ),
						'dialog_no'             => esc_html__( 'Cancel', LEVELUP_TEXT_DOMAIN ),
						'dialog_yes'            => esc_html__( 'Yes, import!', LEVELUP_TEXT_DOMAIN ),
						'selected_import_title' => esc_html__( 'Selected demo import:', LEVELUP_TEXT_DOMAIN ),
					),
					'dialog_options' => apply_filters( 'levelup_import/confirmation_dialog_options', array() )
				)
			);

			wp_enqueue_style( 'ocdi-main-css', LEVELUP_OCDI_URL . 'assets/css/main.css', array() , LEVELUP_VERSION );
		}
	}
	...
	public function import_demo_data_ajax_callback() {
		...
		ini_set( 'memory_limit', apply_filters( 'levelup_import/import_memory_limit', '350M' ) );
		...
		if ( ! $use_existing_importer_data ) {
			 ...
			 * Manually uploaded import files or predefined import files via filter: levelup_import/import_files
			 */
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', LEVELUP_TEXT_DOMAIN );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: levelup_import/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', LEVELUP_TEXT_DOMAIN )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', LEVELUP_TEXT_DOMAIN ),
				...
				wp_send_json( esc_html__( 'No import files specified!', LEVELUP_TEXT_DOMAIN ) );
			}
		}
		...
		if ( ! $this->before_import_executed ) {
			...
			/**
			 * 2). Execute the actions hooked to the 'levelup_import/before_content_import_execution' action:
			 *
			 * Default actions:
			 * 1 - Before content import WP action (with priority 10).
			 */
			do_action( 'levelup_import/before_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
		 ...
		 */
		do_action( 'levelup_import/after_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		...
		if ( false !== has_action( 'levelup_import/after_all_import_execution' ) ) {
	...
	public function import_customizer_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			do_action( 'levelup_import/customizer_import_execution', $this->selected_import_files );
		}
	...
	public function after_all_import_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			do_action( 'levelup_import/after_all_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
	...
	private function final_response() {
		...
		if ( empty( $this->frontend_error_messages ) ) {
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', LEVELUP_TEXT_DOMAIN ),
	...
	public function setup_plugin_with_filter_data() {
		...
		$this->import_files = Helpers::validate_import_file_info( apply_filters( 'levelup_import/import_files', array() ) );

		/**
		 * Register all default actions (before content import, widget, customizer import and other actions)
		 * to the 'before_content_import_execution' and the 'levelup_import/after_content_import_execution' action hook.
		 */
		$import_actions = new ImportActions();
		$import_actions->register_hooks();

		// Importer options array.
		$importer_options = apply_filters( 'levelup_import/importer_options', array(
			'fetch_attachments' => true,
		) );

		// Logger options for the logger used in the importer.
		$logger_options = apply_filters( 'levelup_import/logger_options', array(
