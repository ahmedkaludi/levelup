...
class WPCLICommands extends \WP_CLI_Command {
	...
	public function list_predefined() {
		if ( empty( $this->ocdi->import_files ) ) {
			WP_CLI::error( esc_html__( 'There are no predefined demo imports for currently active theme!', LEVELUP_TEXT_DOMAIN ) );
		}

		WP_CLI::success( esc_html__( 'Here are the predefined demo imports:', LEVELUP_TEXT_DOMAIN ) );
	...
	public function import( $args, $assoc_args ) {
		if ( ! $this->any_import_options_set( $assoc_args ) ) {
			WP_CLI::error( esc_html__( 'At least one of the possible options should be set! Check them with --help', LEVELUP_TEXT_DOMAIN ) );
		}
	...
	/**
	 * Import the predefined demo content/widgets/customizer settings with levelup_import.
	 *
	...
	private function import_predefined( $predefined_index ) {
		if ( ! is_numeric( $predefined_index ) ) {
			WP_CLI::error( esc_html__( 'The "predefined" parameter should be a number (an index of the OCDI predefined demo import)!', LEVELUP_TEXT_DOMAIN ) );
		}

		$predefined_index = absint( $predefined_index );

		if ( ! array_key_exists( $predefined_index, $this->ocdi->import_files ) ) {
			WP_CLI::warning( esc_html__( 'The supplied predefined index does not exist! Please take a look at the available predefined demo imports:', LEVELUP_TEXT_DOMAIN ) );
		...
		WP_CLI::log( esc_html__( 'Predefined demo import started! All other parameters will be ignored!', LEVELUP_TEXT_DOMAIN ) );

		$selected_files = $this->ocdi->import_files[ $predefined_index ];

		if ( ! empty( $selected_files['import_file_name'] ) ) {
			WP_CLI::log( sprintf( esc_html__( 'Selected predefined demo import: %s', LEVELUP_TEXT_DOMAIN ), $selected_files['import_file_name'] ) );
		}

		WP_CLI::log( esc_html__( 'Preparing the demo import files...', LEVELUP_TEXT_DOMAIN ) );

		$import_files =	Helpers::download_import_files( $selected_files );

		if ( empty( $import_files ) ) {
			WP_CLI::error( esc_html__( 'Demo import files could not be retrieved!', LEVELUP_TEXT_DOMAIN ) );
		}

		WP_CLI::log( esc_html__( 'Demo import files retrieved successfully!', LEVELUP_TEXT_DOMAIN ) );

		WP_CLI::log( esc_html__( 'Importing...', LEVELUP_TEXT_DOMAIN ) );

		if ( ! empty( $import_files['content'] ) ) {
			$this->do_action( 'levelup_import/before_content_import_execution', $import_files, $this->ocdi->import_files, $predefined_index );

			$this->import_content( $import_files['content'] );
		}

		if ( ! empty( $import_files['widgets'] ) ) {
			$this->do_action( 'levelup_import/before_widgets_import', $import_files );
		...
		$this->do_action( 'levelup_import/after_all_import_execution', $import_files, $this->ocdi->import_files, $predefined_index );

		WP_CLI::log( esc_html__( 'Predefined import finished!', LEVELUP_TEXT_DOMAIN ) );
	}

	/**
	 * Import the content with levelup_import.
	 *
	...
	private function import_content( $relative_file_path ) {
		...
		if ( ! file_exists( $content_import_file_path ) ) {
			WP_CLI::warning( esc_html__( 'Content import file provided does not exist! Skipping this import!', LEVELUP_TEXT_DOMAIN ) );
			return false;
		}

		// Change the single AJAX call duration so the whole content import will be done in one go.
		add_filter( 'levelup_import/time_for_one_ajax_call', function() {
			return 3600;
		} );

		WP_CLI::log( esc_html__( 'Importing content (this might take a while)...', LEVELUP_TEXT_DOMAIN ) );

		Helpers::append_to_file( '', $this->ocdi->log_file_path, esc_html__( 'Importing content' , LEVELUP_TEXT_DOMAIN ) );
		...
		if( empty( $this->ocdi->frontend_error_messages ) ) {
			WP_CLI::success( esc_html__( 'Content import finished!', LEVELUP_TEXT_DOMAIN ) );
		}
		else {
			WP_CLI::warning( esc_html__( 'There were some issues while importing the content!', LEVELUP_TEXT_DOMAIN ) );
	...
	/**
	 * Import the widgets with levelup_import.
	 *
	...
	private function import_widgets( $relative_file_path ) {
		...
		if ( ! file_exists( $widgets_import_file_path ) ) {
			WP_CLI::warning( esc_html__( 'Widgets import file provided does not exist! Skipping this import!', LEVELUP_TEXT_DOMAIN ) );
			return false;
		}

		WP_CLI::log( esc_html__( 'Importing widgets...', LEVELUP_TEXT_DOMAIN ) );

		WidgetImporter::import( $widgets_import_file_path );

		if( empty( $this->ocdi->frontend_error_messages ) ) {
			WP_CLI::success( esc_html__( 'Widgets imported successfully!', LEVELUP_TEXT_DOMAIN ) );
		}
		else {
			WP_CLI::warning( esc_html__( 'There were some issues while importing widgets!', LEVELUP_TEXT_DOMAIN ) );
	...
	/**
	 * Import the customizer settings with levelup_import.
	 *
	...
	private function import_customizer( $relative_file_path ) {
		...
		if ( ! file_exists( $customizer_import_file_path ) ) {
			WP_CLI::warning( esc_html__( 'Customizer import file provided does not exist! Skipping this import!', LEVELUP_TEXT_DOMAIN ) );
			return false;
		}

		WP_CLI::log( esc_html__( 'Importing customizer settings...', LEVELUP_TEXT_DOMAIN ) );

		CustomizerImporter::import( $customizer_import_file_path );

		if( empty( $this->ocdi->frontend_error_messages ) ) {
			WP_CLI::success( esc_html__( 'Customizer settings imported successfully!', LEVELUP_TEXT_DOMAIN ) );
		}
		else {
			WP_CLI::warning( esc_html__( 'There were some issues while importing customizer settings!', LEVELUP_TEXT_DOMAIN ) );
	...
	private function do_action( $action, $import_files = array(), $all_import_files = array(), $selected_index = null ) {
		if ( false !== has_action( $action ) ) {
			WP_CLI::log( sprintf( esc_html__( 'Executing action: %s ...', LEVELUP_TEXT_DOMAIN ), $action ) );
