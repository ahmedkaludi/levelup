(function( $, wpcustomize ) {
		 ...
		 var is_rtl = HF_Builder.is_rtl;
	    var HFpanelBuilder = function( options, id ){
	        ...
	        var Builder = {
	            ...
	            init: function( controlId, items, devices, settingsId ){
	                var that = this;


	                var template = that.getTemplate();
	                var template_id =  'template-headerfooter--builder-panel';
	                var html = template( options , template_id );
	                that.container = $( html );
	                $( 'body .wp-full-overlay' ).append( that.container );
	                that.controlId = controlId;
	                that.settingsId = settingsId;
	                that.items = items;
	        ...
	        Builder.init( options.control_id, options.items, options.devices, options.settings );
	        return Builder;
	    };
...
( function( $, api ) {
    ...
    var header_changed = function( partial_id, remove_items ){
        ...
        $document.trigger( 'header_panel_settings_changed',[ partial_id ] );
    };
    ...
    wp.customize( 'header_panel_settings', function( settings ) {
        settings.bind( function( to ) {
        	//console.log(settings.partial.id);
            header_changed( 'header_panel', false );
            $document.trigger('customize_section_opened', [ 'header_sidebar' ]) ;
            if ( to.indexOf( 'menu_sidebar_dropdown' ) > 1 ) {
                $( '.menu-mobile-toggle, .menu-mobile-toggle .hamburger' ).addClass( 'is-active' );
            } else {
    ...
    wp.customize.bind( 'preview-ready', function() {
        ...
        wp.customize.selectiveRefresh.bind( 'partial-content-rendered', function( settings ) {
            console.log( 'settings.partial.id', settings );
